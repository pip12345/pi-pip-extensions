...
export class HttpTransport extends EventEmitter {
  ...
  send(message: JsonRpcMessage): void {
    ...
    if (isInitializedNotification(message)) {
      operation = this.postMessage(message).then(() => {
      ...
      operation = this.initializedPost.then(() => this.postMessage(message));
    } else {
      operation = this.postMessage(message);
    }

    void operation.catch((error) => {
      if (!this.closed) this.emit("error", normalizeError(error));
    });
  }
  ...
  private async postMessage(message: JsonRpcMessage): Promise<void> {
    if (this.legacyOnly || this.mode === "legacy") return this.postLegacy(message);
    return this.postStreamable(message);
  }

  private async postStreamable(message: JsonRpcMessage): Promise<void> {
  ...
  private async startLegacySse(): Promise<void> {
