...
A subagent is a child task run with isolated context. The caller must put all needed context in the prompt. Subagents are not permanent assistants and are not restored after pi restarts.
...
Ephemeral subagents cannot be continued after completion. Use `keep:true` for reusable runs, or enable **Always keep** in `/pip-settings`.
