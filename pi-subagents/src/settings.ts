...
export function registerSubagentSettings(): void {
  registerSettingsSection({
    ...
    settings: {
      ...
      ephemeralTtlMinutes: setting.number({ label: "Ephemeral TTL", default: 30, min: 1, max: 1440, step: 1, order: 2, description: "Minutes to retain completed non-kept subagents for read/view inspection." }),
      maxRecentPerParent: setting.number({ label: "Max recent", default: 20, min: 1, max: 200, step: 1, order: 3, description: "Maximum completed non-kept subagents retained per parent session." }),
