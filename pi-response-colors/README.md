...
Square-bracket tags such as `[yellow]...[/yellow]` are also supported, so existing sessions still render. Tag names are case-insensitive. Openers and closers must use the same color and bracket style; `<yellow>...[/yellow]` stays literal.

The extension teaches only the XML-style syntax to the model in interactive mode; prompt profiles decide when colors are useful. During rendering it converts balanced tags to standard terminal foreground colors. ANSI is never added to the saved assistant message.
