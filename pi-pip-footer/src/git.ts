...
export function renderLocation(ctx: any, theme: any, gitState: GitState | null, settings: ScopedSettings): string {
  const cwdSetting = settings.get<"off" | "project" | "path">("showCwd", "project");
  const parts: string[] = [];
  if (cwdSetting !== "off") {
    const home = homedir();
  ...
  if (settings.get("showGit", false) && gitState?.branch) {
