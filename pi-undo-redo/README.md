...
Safety: `/undo` refuses unless the target prompt is at the true end of a branch. It will not delete hidden downstream history after `/tree` navigation. Backups are stored under `~/.pi/agent/pip/backup/undo-redo/`; the newest 25 are retained and files older than seven days are pruned.
